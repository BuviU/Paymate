<?php
/**
 * Plugin Name: EboxLMS Multisite
 * Plugin URI: https://arttricks.com.au/
 * Description: Ebox LMS Plugin - Turn your WordPress site into a learning management system.
 * Version: 1.0.0
 * Author: Arttricks
 * Author URI: https://arttricks.com.au/
	...
	class LD_Multisite {
		...
		public function add_signup_meta( $user_signup_meta = array() ) {
			if ( true === $this->doing_signup ) {
				$user_signup_meta['ebox_meta'] = array();
				...
				if ( ( isset( $_POST['ebox-registration-form'] ) ) && ( wp_verify_nonce( $_POST['ebox-registration-form'], 'ebox-registration-form' ) ) ) {
					if ( ( isset( $_POST['ebox-registration-form-post'] ) ) && ( ! empty( $_POST['ebox-registration-form-post'] ) ) ) {
						$course_id = absint( $_POST['ebox-registration-form-post'] );
						if ( ( isset( $_POST['ebox-registration-form-post-nonce'] ) ) && ( wp_verify_nonce( $_POST['ebox-registration-form-post-nonce'], 'ebox-registration-form-post-' . $course_id . '-nonce' ) ) ) {
							$user_signup_meta['ebox_meta']['_ld_registered_post'] = $course_id;
						}
					}
				}

				if ( isset( $_POST['blog_id'] ) ) {
					$user_signup_meta['ebox_meta']['blog_id'] = $_POST['blog_id'];
				}

				if ( isset( $_POST['redirect_to'] ) ) {
					$user_signup_meta['ebox_meta']['redirect_to'] = $_POST['redirect_to'];
				}
			}
		...
		public function wpmu_activate_user( $user_id = 0, $password = '', $meta = array() ) {
			if ( ( is_multisite() ) && ( ! empty( $user_id ) ) ) {
				if ( ( isset( $meta['ebox_meta'] ) ) && ( is_array( $meta['ebox_meta'] ) ) && ( ! empty( $meta['ebox_meta'] ) ) ) {
					$this->activated_user_id = absint( $user_id );
					foreach ( $meta['ebox_meta'] as $key => $val ) {
						update_user_meta( $user_id, $key, $val );
					}

					if ( ( isset( $meta['ebox_meta']['blog_id'] ) ) && ( ! empty( $meta['ebox_meta']['blog_id'] ) ) ) {
						$default_role = get_site_option( absint( $meta['ebox_meta']['blog_id'] ), 'default_role' );
						if ( ! empty( $default_role ) ) {
							if ( apply_filters( 'ebox_multisite_user_active_add_to_blog', true, $user_id, absint( $meta['ebox_meta']['blog_id'] ), $default_role ) ) {
								add_user_to_blog( absint( $meta['ebox_meta']['blog_id'] ), $user_id, $default_role );
							}
						}
					}
				}
