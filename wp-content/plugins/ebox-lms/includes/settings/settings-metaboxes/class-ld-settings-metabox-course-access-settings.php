...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Course_Access_Settings' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Course Access Settings.
	 *
	 * @since 3.0.0
	 */
	class ebox_Settings_Metabox_Course_Access_Settings extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_metabox_key = 'ebox-course-access-settings';

			// Section label/header.
			$this->settings_section_label = sprintf(
				// translators: placeholder: Course.
				esc_html_x( '%s Access Settings', 'placeholder: Course', 'ebox' ),
				ebox_get_custom_label( 'course' )
			);

			$this->settings_section_description = sprintf(
				// translators: placeholder: course.
				esc_html_x( 'Controls how users will gain access to the %s', 'placeholder: course', 'ebox' ),
		...
		public function load_settings_values() {
			...
			if ( true === $this->settings_values_loaded ) {
				...
				if ( ( ! isset( $this->setting_option_values['course_price_type'] ) ) || ( empty( $this->setting_option_values['course_price_type'] ) ) ) {
					$this->setting_option_values['course_price_type'] = ebox_DEFAULT_COURSE_PRICE_TYPE;
				}
		...
		public function load_settings_fields() {
			...
			if ( ebox_use_select2_lib() ) {
				$select_course_options_default = sprintf(
					// translators: placeholder: course.
					esc_html_x( 'Search or select a %s', 'placeholder: course', 'ebox' ),
					ebox_get_custom_label( 'course' )
				);
				...
				$select_course_options_default = array(
					'' => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Select %s', 'placeholder: course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
				);
				$select_course_options         = $ebox_lms->select_a_course();
			...
			$this->setting_option_fields = array(
				'course_price_type_paynow_price'          => array(
					...
					'label'   => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Price', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'    => 'text',
					'class'   => '-medium',
					...
					'rest'    => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: Course.
								'description' => sprintf( esc_html_x( 'Pay Now %s Price', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) ),
								'type'        => 'string',
								'default'     => '',
							),
						),
				...
				'course_price_type_paynow_enrollment_url' => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Enrollment URL', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'      => 'url',
					'class'     => 'full-text',
					...
					'help_text' => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Enter the URL of the page you want to redirect your enrollees after signing up for this specific %s', 'placeholder: course', 'ebox' ),
						ebox_get_custom_label_lower( 'course' )
					),
					'default'   => '',
					'rest'      => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: course.
								'description' => sprintf( esc_html_x( 'Pay Now %s Enrollment URL', 'placeholder: course', 'ebox' ), ebox_get_custom_label_lower( 'course' ) ),
								'type'        => 'string',
								'default'     => '',
							),
						),
			...
			$this->setting_option_fields = array(
					...
					'rest'    => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: Course.
								'description' => sprintf( esc_html_x( 'Subscribe %s Price', 'placeholder: Course', 'ebox' ), ebox_get_custom_label( 'course' ) ),
								'type'        => 'string',
								'default'     => '',
							),
						),
				...
				'course_price_type_subscribe_billing_cycle' => array(
					...
					'html'  => ebox_billing_cycle_setting_field_html(
						0,
						ebox_get_post_type_slug( 'course' )
					),
				),
				...
				'course_trial_price'                => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Trial Price', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'      => 'text',
					'class'     => '-medium',
					...
					'help_text' => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Enter the price for the trial period for this %s', 'placeholder: course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'default'   => '',
					'rest'      => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: course.
								'description' => sprintf( esc_html_x( '%s Trial Price', 'placeholder: course', 'ebox' ), ebox_get_custom_label( 'course' ) ),
								'type'        => 'string',
								'default'     => '',
							),
						),
				...
				'course_trial_duration'             => array(
					...
					// translators: placeholder: course.
					'help_text' => sprintf( esc_html_x( 'The length of the trial period, after the trial is over, the normal %s price billing goes into effect.', 'placeholder: course', 'ebox' ), ebox_get_custom_label_lower( 'course' ) ),
					'default'   => '',
				),
					...
					'rest'      => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: course.
								'description' => sprintf( esc_html_x( 'Subscribe %s Enrollment URL', 'placeholder: course', 'ebox' ), ebox_get_custom_label_lower( 'course' ) ),
								'type'        => 'string',
								'default'     => '',
							),
						),
			...
			$this->setting_option_fields = array(
					...
					'rest'    => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: Course.
								'description' => sprintf( esc_html_x( 'Closed %s Price', 'placeholder: Course', 'ebox' ), ebox_get_custom_label( 'course' ) ),
								'type'        => 'string',
								'default'     => '',
							),
						),
				...
				'course_price_type_closed_custom_button_url' => array(
					...
					'help_text' => sprintf(
						...
						esc_html_x( 'Redirect the "%s" button to a specific URL.', 'placeholder: "Take this Course" button label', 'ebox' ),
						ebox_get_custom_label( 'button_take_this_course' )
					),
					'default'   => '',
					'rest'      => array(
						'show_in_rest' => ebox_REST_API::enabled(),
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: Course.
								'description' => sprintf( esc_html_x( 'Closed %s Button URL', 'placeholder: Course', 'ebox' ), ebox_get_custom_label( 'course' ) ),
								'type'        => 'string',
								'default'     => '',
							),
						),
			...
			$this->setting_option_fields = array(
				'course_price_type'             => array(
					...
					'default' => ebox_DEFAULT_COURSE_PRICE_TYPE,
					'options' => array(
						'open'      => array(
							'label'       => esc_html__( 'Open', 'ebox' ),
							'description' => sprintf(
								// translators: placeholder: course.
								esc_html_x( 'The %s is not protected. Any user can access its content without the need to be logged-in or enrolled.', 'placeholder: course', 'ebox' ),
								ebox_get_custom_label_lower( 'course' )
							),
						),
						'free'      => array(
							...
							'description' => sprintf(
								// translators: placeholder: course.
								esc_html_x( 'The %s is protected. Registration and enrollment are required in order to access the content.', 'placeholder: course', 'ebox' ),
								ebox_get_custom_label_lower( 'course' )
							),
						),
						'paynow'    => array(
							...
							'description'         => sprintf(
								...
								esc_html_x( 'The %1$s is protected via the ebox built-in PayPal and/or Stripe. Users need to purchase the %2$s (one-time fee) in order to gain access.', 'placeholder: course, course', 'ebox' ),
								ebox_get_custom_label_lower( 'course' ),
								ebox_get_custom_label_lower( 'course' )
							),
							'inline_fields'       => array(
						...
						'subscribe' => array(
							...
							'description'         => sprintf(
								// translators: placeholder: course, course.
								esc_html_x( 'The %1$s is protected via the ebox built-in PayPal and/or Stripe. Users need to purchase the %2$s (recurring fee) in order to gain access.', 'placeholder: course, course', 'ebox' ),
								ebox_get_custom_label_lower( 'course' ),
								ebox_get_custom_label_lower( 'course' )
							),
							'inline_fields'       => array(
						...
						'closed'    => array(
							...
							'description'         => sprintf(
								...
								esc_html_x( 'The %1$s can only be accessed through admin enrollment (manual), %2$s enrollment, or integration (shopping cart or membership) enrollment. No enrollment button will be displayed, unless a URL is set (optional).', 'placeholder: course', 'ebox' ),
								ebox_get_custom_label_lower( 'course' ),
					...
					'rest'    => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: Course.
								'description' => sprintf( esc_html_x( '%s Price Type', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) ),
								'type'        => 'string',
								'default'     => 'open',
								'enum'        => array(
									'open',
				...
				'course_prerequisite_compare'   => array(
					...
					'label'          => esc_html__( 'Compare Mode', 'ebox' ),
					'type'           => 'radio',
					'default'        => 'ANY',
					'value'          => $this->setting_option_values['course_prerequisite_compare'],
					'options'        => array(
						'ANY' => array(
							'label'       => esc_html__( 'Any Selected', 'ebox' ),
							'description' => sprintf(
								// translators: placeholder: courses, course.
								esc_html_x( 'The user must complete any one of the selected %1$s in order to access this %2$s', 'placeholder: course, course', 'ebox' ),
								ebox_get_custom_label_lower( 'courses' ),
								ebox_get_custom_label_lower( 'course' )
							),
						),
						'ALL' => array(
							'label'       => esc_html__( 'All Selected', 'ebox' ),
							'description' => sprintf(
								// translators: placeholder: course, course.
								esc_html_x( 'The user must complete all selected %1$s in order to access this %2$s', 'placeholder: course, course', 'ebox' ),
								ebox_get_custom_label_lower( 'course' ),
								ebox_get_custom_label_lower( 'course' )
							),
						),
				...
				'course_prerequisite'           => array(
					...
					'label'          => sprintf(
						...
						esc_html_x( '%s to Complete', 'placeholder: courses', 'ebox' ),
				...
				'course_points_enabled'         => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Points', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'                => 'checkbox-switch',
				...
				'course_points_access'          => array(
					...
					'label'          => esc_html__( 'Required for Access', 'ebox' ),
					'type'           => 'number',
					'value'          => $this->setting_option_values['course_points_access'],
					'default'        => 0,
					'class'          => 'small-text',
					'input_label'    => esc_html__( 'point(s)', 'ebox' ),
					'input_error'    => esc_html__( 'Value should be zero or greater with up to 2 decimal places.', 'ebox' ),
					...
					'help_text'      => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Number of points required in order to gain access to this %s.', 'placeholder: course.', 'ebox' ),
						ebox_get_custom_label_lower( 'course' )
					),
					'rest'           => array(
						'show_in_rest' => ebox_REST_API::enabled(),
				...
				'course_points'                 => array(
					'name'           => 'course_points',
					'label'          => esc_html__( 'Awarded on Completion', 'ebox' ),
					'type'           => 'number',
					'step'           => 'any',
					'min'            => '0',
					...
					'help_text'      => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Number of points awarded for completing this %s.', 'placeholder: course.', 'ebox' ),
						ebox_get_custom_label_lower( 'course' )
					),
					'input_error'    => esc_html__( 'Value should be zero or greater with up to 2 decimal places.', 'ebox' ),
					'attrs'          => array(
				...
				'expire_access'                 => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Access Expiration', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'                => 'checkbox-switch',
					'options'             => array(
				...
				'expire_access_days'            => array(
					...
					'label'          => esc_html__( 'Access Period', 'ebox' ),
					'type'           => 'number',
					'class'          => 'small-text',
					'value'          => $this->setting_option_values['expire_access_days'],
					'input_label'    => esc_html__( 'days', 'ebox' ),
					...
					'help_text'      => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Set the number of days a user will have access to the %s from enrollment date.', 'placeholder: course.', 'ebox' ),
						ebox_get_custom_label_lower( 'course' )
					),
					'rest'           => array(
						'show_in_rest' => ebox_REST_API::enabled(),
				...
				'expire_access_delete_progress' => array(
					...
					'label'          => esc_html__( 'Data Deletion', 'ebox' ),
					'type'           => 'checkbox-switch',
					'options'        => array(
						'on' => sprintf(
							// translators: placeholder: course.
							esc_html_x( 'All user %s data will be deleted upon access expiration', 'placeholder: course.', 'ebox' ),
							ebox_get_custom_label_lower( 'course' )
						),
						''   => '',
					),
					...
					'help_text'      => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Delete the user\'s %1$s and %2$s data when the %3$s access expires.', 'placeholder: course, quiz, course.', 'ebox' ),
						ebox_get_custom_label_lower( 'course' ),
				...
				'course_access_list_enabled'    => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Course.
						esc_html_x( 'Alter %s Access List', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'                => 'checkbox-switch',
					'options'             => array(
						'on' => sprintf(
							// translators: placeholder: Course.
							esc_html_x( 'You can change the LD-%s enrollees by user ID (Proceed with caution)', 'placeholder: Course', 'ebox' ),
							ebox_get_custom_label( 'course' )
						),
						''   => '',
					),
					...
					'help_text'           => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Displays a list of %s enrollees by user ID. Note that not all enrollees may be reflected. We do not recommend editing this field.', 'placeholder: course.', 'ebox' ),
						ebox_get_custom_label_lower( 'course' )
					),
				),
				'course_access_list'            => array(
					...
					'placeholder'    => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Add a comma-list of user IDs to grant access to this %s', 'placeholder: course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'attrs'          => array(
						'rows' => '2',
			...
			if ( is_a( $this->_post, 'WP_Post' ) ) {
				$course_teams = ebox_get_course_teams( $this->_post->ID, true );
				if ( ( ! empty( $course_teams ) ) && ( 'closed' !== $this->setting_option_values['course_price_type'] ) ) {
					$alert_message = '';

					if ( 1 === count( $course_teams ) ) {
						$alert_message = sprintf(
							// translators: placeholders, course, teams, Team, course, teams.
							esc_html_x( 'This %1$s is a part of a %2$s. %3$s settings will override %4$s settings for any user enrolled in the %5$s.', 'placeholders, course, team, Team, course, team.', 'ebox' ),
							ebox_get_custom_label_lower( 'course' ),
							ebox_get_custom_label_lower( 'team' ),
					...
					} elseif ( 1 < count( $course_teams ) ) {
						$alert_message = sprintf(
							// translators: placeholders, course, teams, Team, course, teams.
							esc_html_x( 'This %1$s is a part of multiple %2$s. %3$s settings will override %4$s settings for any user enrolled in the %5$s.', 'placeholders, course, teams, Team, course, teams.', 'ebox' ),
							ebox_get_custom_label_lower( 'course' ),
					...
					if ( ! empty( $alert_message ) ) {
						$this->setting_option_fields = array_merge(
							array(
								'course_price_type_team_alert' => array(
									'name'       => 'course_price_type_team_alert',
									'type'       => 'html',
									'input_full' => true,
		...
		public function filter_saved_fields( $settings_values = array(), $settings_metabox_key = '', $settings_screen_id = '' ) {
			if ( ( $settings_screen_id === $this->settings_screen_id ) && ( $settings_metabox_key === $this->settings_metabox_key ) ) {
				...
				if ( isset( $_POST['course_price_billing_t3'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
					...
					$settings_values['course_price_billing_t3'] = ebox_billing_cycle_field_frequency_validate( $settings_values['course_price_billing_t3'] );
				}
				...
				if ( isset( $_POST['course_price_billing_p3'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
					...
					$settings_values['course_price_billing_p3'] = ebox_billing_cycle_field_interval_validate( $settings_values['course_price_billing_p3'], $settings_values['course_price_billing_t3'] );
				}
				...
				if ( isset( $_POST['course_trial_duration_t1'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
					...
					$settings_values['course_trial_duration_t1'] = ebox_billing_cycle_field_frequency_validate( $settings_values['course_trial_duration_t1'] );
				}
				...
				if ( isset( $_POST['course_trial_duration_p1'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
					...
					$settings_values['course_trial_duration_p1'] = ebox_billing_cycle_field_interval_validate( $settings_values['course_trial_duration_p1'], $settings_values['course_trial_duration_t1'] );
				}
				...
				/**
				 * Check the modules Per Page set course_prerequisite_enabled/course_prerequisite. If 'course_prerequisite_enabled' setting is
				...
				/**
				 * Check the modules Per Page set expire_access/expire_access_days. If 'expire_access' setting is
				...
				/**
				 * Filters ebox settings save values.
				 *
	...
	add_filter(
		...
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Course_Access_Settings'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Course_Access_Settings' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Course_Access_Settings'] = ebox_Settings_Metabox_Course_Access_Settings::add_metabox_instance();
			}
