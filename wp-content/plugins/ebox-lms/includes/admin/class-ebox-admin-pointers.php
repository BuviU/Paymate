...
if ( ! class_exists( 'ebox_Admin_Pointers' ) ) {
	/**
	 * ebox Admin Pointers Class
	 *
	 * @since 3.0.0
	 */
	class ebox_Admin_Pointers {
		/**
		...
		public function admin_enqueue_scripts() {
			...
			wp_enqueue_script(
				'ebox-admin-pointer-script',
				ebox_LMS_PLUGIN_URL . 'assets/js/ebox-admin-pointers' . ebox_min_asset() . '.js',
				array( 'jquery' ),
		...
		public function register_pointers() {
			 ...
			 */
			$pointers = apply_filters( 'ebox_screen_pointers', array(), $this->screen_id );
...
add_action(
	'ebox_admin_init',
	function() {
		new ebox_Admin_Pointers();
	}
);

add_filter(
	'ebox_screen_pointers',
	function( $pointers = array(), $screen_id = '' ) {
		...
		if ( ebox_is_admin_user() ) {
			if ( 'new' === $ld_prior_version ) {
				if ( ! isset( $pointers['ebox-new-install'] ) ) {

					$setup_page_slug = ebox_Settings_Page_Setup::SETUP_SLUG;

					if ( ebox_cloud_is_enabled() ) {
						$setup_page_slug = ebox_Settings_Page_Setup::SETUP_SLUG_CLOUD;
					}

					$pointers['ebox-new-install'] = array(
						'id'       => 'ebox-new-install',
						'screen'   => '',
						'target'   => '#toplevel_page_ebox-lms .wp-menu-name',
						'title'    => '<span id="ld-pointer-title-ebox-new-install" class="ld-pointer-title">' . esc_html__( 'First time using Ebox LMS by Arttricks?', 'ebox' ) . '</span>',
						'content'  => '<span class="ld-pointer-content">' . sprintf(
							// translators: placeholder: Link to Setup page.
							esc_html_x( 'Go to the Ebox LMS by Arttricks %s', 'placeholder: Link to Setup page', 'ebox' ),
							'<a href="' . admin_url( 'admin.php?page=' ) . $setup_page_slug . '">' . esc_html__( 'setup', 'ebox' ) . '</a>'
						) . '</span>',
						'position' => array(
